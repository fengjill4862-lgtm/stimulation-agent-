#!/usr/bin/env python3
"""Function 0: preview and apply RHS folder renames derived from the stim waveform.

Paste either a parent folder containing RHS session subfolders, or one RHS
session folder. Always use Preview Rename Plan before Apply Renames: the
generated suffix is a waveform-derived summary, and the refractory period in
particular is inferred from the recorded data rather than read from an explicit
Intan configuration field.
"""
...
from wideband_ui_common import default_data_dir, folder_path_from_text

from rename_rhs_folders_by_stim_waveform import apply_rename_plan, plan_renames

# Only seeds the UI text box; any parent folder can be pasted in.
EXAMPLE_RENAME_PARENT_DIR = Path(
...
def rename_plan_table(plans) -> HTML:
    ...
    for plan in plans:
        ...
        color = _STATUS_COLORS.get(plan.status, "#333333")
        rows.append(
            "<tr>"
...
def show_function0_rename_rhs_folders(
    namespace: MutableMapping[str, object] | None = None,
) -> None:
    """Display Function 0 widgets inside VS Code/Jupyter."""
    parent_dir = default_data_dir(
        namespace,
        keys=(),
        fallback=EXAMPLE_RENAME_PARENT_DIR,
        publish_as="DEFAULT_RENAME_PARENT_DIR",
    )
    ...
    rename_state: dict[str, list] = {"plans": []}
    ...
    def preview_rename_plan(_button=None) -> None:
        ...
        with rename_output:
            ...
            try:
                parent_folder = folder_path_from_text(rename_parent_text.value)
                plans = plan_renames(parent_folder)
            except Exception as exc:
                rename_state["plans"] = []
                rename_status.value = (
                    f"<b style='color:#b00020'>Preview failed:</b> {html.escape(str(exc))}"
                )
                return
...
__all__ = ["show_function0_rename_rhs_folders"]
